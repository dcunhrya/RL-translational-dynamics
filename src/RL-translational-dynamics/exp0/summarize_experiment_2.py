...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--handoff-results-dir", type=Path, default=Path("results/raw/experiment_2_fixed_handoff"))
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("results/processed/experiment_2_fixed_handoff"))
...
def latest_handoff_runs(results_dir: Path) -> dict[tuple[str, int, int], RunSummary]:
...
def plot_env_learning_curves(
    env: str,
    handoff_runs: dict[tuple[str, int, int], RunSummary],
    ...
    for switch_fraction in switch_fractions:
        ...
        run_dirs = []
        for seed in seeds:
            summary = handoff_runs.get((env, seed, switch_step))
        ...
        label = f"SAC->{int(switch_fraction * 100)}% switch"
    ...
    for algorithm, style, color in [("sac", ":", "#444444"), ("ppo", "-.", "#999999")]:
        run_dirs = []
        for seed in [0, 1]:
    ...
    ax.set_title(f"{env}: fixed handoff learning curves")
    ax.set_xlabel("Env steps")
    ax.set_ylabel("Eval return")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    output_path = output_dir / f"{env.replace('-', '_')}_handoff_learning_curves.png"
...
def plot_final_returns(
    ...
    labels = ["SAC", "25%", "50%", "75%", "PPO"]
    x = np.arange(len(labels))
    width = 0.35
    colors = {"sac": "#444444", 0.25: "#1f77b4", 0.5: "#ff7f0e", 0.75: "#2ca02c", "ppo": "#999999"}
    ...
    for ax, env in zip(axes, envs):
        ...
        bar_colors = [colors["sac"], colors[0.25], colors[0.5], colors[0.75], colors["ppo"]]
    ...
    fig.suptitle("Experiment 2: fixed handoff final return comparison", fontsize=14)
    fig.tight_layout()
    output_path = output_dir / "handoff_final_return_comparison.png"
    fig.savefig(output_path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return output_path
...
def write_markdown_summary(
    ...
    lines = [
        "# Experiment 2 Fixed Handoff Summary",
        "",
        ...
        for switch_fraction in switch_fractions:
            ...
            for seed in seeds:
                ...
                if summary is not None:
                    ...
                    if val is not None:
                        vals.append(val)
            lines.append(f"- Handoff {int(switch_fraction * 100)}%: {np.mean(vals):.2f} +/- {np.std(vals):.2f}")
        lines.append(f"- PPO baseline: {np.mean(ppo_vals):.2f} +/- {np.std(ppo_vals):.2f}")
        lines.append("")

    output_path = output_dir / "experiment_2_summary.md"
...
def main() -> None:
    ...
    handoff_runs = latest_handoff_runs(args.handoff_results_dir)
    baseline_runs = latest_baseline_runs(args.baseline_results_dir)

    generated = []
    for env in args.envs:
        generated.append(
            plot_env_learning_curves(env, handoff_runs, baseline_runs, args.seeds, args.switch_fractions, args.output_dir)
        )
    generated.append(
        plot_final_returns(args.envs, handoff_runs, baseline_runs, args.seeds, args.switch_fractions, args.output_dir)
    )
    generated.append(
        write_markdown_summary(args.envs, handoff_runs, baseline_runs, args.seeds, args.switch_fractions, args.output_dir)
    )
